...
class Cai extends CI_Controller {
    ...
    public function index() {
        ...
        if ($loginResult['loginState'] === Constants::S_AUTH && $loginResult['userinfo']['openId'] && !empty($body['id'])) {
            ...
            $zanArray = DB::row('t_cai', ['*'], $conditions);
            //如果没有数据，说明是点赞，并且要新增数据
            if (empty($zanArray)) {
                $f_del_flag = 1;
                //在记录表中加入数据
                DB::insert('t_cai', compact('f_open_id', 'f_word_id', 'f_del_flag'));
            } else {
                $f_del_flag = empty($zanArray->f_del_flag) ? 1 : 0;
                DB::update(
                        't_cai', compact('f_del_flag'), compact('f_open_id', 'f_word_id')
                );
            }
            //f_word表增加或者减少赞的数量
            $f_zan = empty($f_del_flag) ? 'f_cai-1' : 'f_cai+1';
            $f_id = $f_word_id;
            $sql = "UPDATE `t_word` SET f_cai = ".$f_zan." WHERE f_id = :f_id ";
            DB::raw(
                    $sql, array(':f_id'=>$f_id)
            );
